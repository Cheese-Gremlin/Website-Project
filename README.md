...
goals:
    my main goal through this project is to get comfortabe using AI. so far i have had a very oposed mindset towards AI so i want to use this course/ project to interact with it more and hopefully build a more open response to it and learn how it can help us grow as designers or even humans. 
...
# creating immages with AI 
using gemini - nano banana 2 

notes:
    slight 'moral dilema' about gemini logo. I would like to remove it to make my website looks cleaner but it feels incorect and almost 'cheaty/ lying' to fully remove it. from the beggining i was planning to add a small text somewhere on my website (in the settings and info page) that most of the code is generated by AI so I will now add a part that mentions that some images that make up the website are created by AI. i would really love to (and prefer to) do all drawing/ make all images myself but due to the scope of my project and the time limit, i wont be able to and will need the help of AI. I will still draw what I can and always make a rough sketch for the AI to follow so that im doing most of the creative stuff and only have the AI do the 'colouring and pixel style'. My current plan is to try to blend the gemini logo/ watermark into the background e.g. turn it into a leaf/ cloud ect.

landing page background image
prompt:
    can you please take the 'home-page-castle-drawing' image and turn it into a pixel art style using the mood board. I want it to be a grey stone medieval castle with moss and vines growing on it and a stained glass window above the door. make the door a wood colour.
------iamages shown---------
response:
...
---------innsert iage-----
opinion:
    -very happy with the outcome. it has exactly done what i asked and perfectly provided what i invisioned.
...
prompt 1.3:
    awesome! can you please zoom out of the image a small bit so that more of the sides, sky and foreground can be seen
response:
-------insert image--------
opinion: 
    i like it but want the towers it made to be more syetrical and the 'courtyard' to be grass
prompt 1.4:
    wonderful. can you please make both towers the same so the image is a little more symmetrical. can you also make the courtyard grass instead of stone.
response:
-----insert image-----
opinion:
    very happy with it, it has the gemini logo in the corner which i dont reallly like so i may edit the immage to remove the logo or make it look like part of the plants
